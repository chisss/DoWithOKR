...
```text
.okr/
  active.md          # 当前 OKR 主文件（GM OKR、角色树、角色 OKR、交付幕计划）
  status.md          # KR 状态看板
...
当用户在断点续跑时选择"重新开始"，当前状态文件会被归档到 `.okr/archive/` 目录，保留历史快照供回溯参考。

### 归档规则

- 归档由 `okr-run` 在用户选择"重新开始"时自动执行。
- 归档前创建 `.okr/archive/` 目录（如不存在）。
- 归档文件命名规则：`<YYYY-MM-DD>-<原文件名>.md`，例如 `2026-04-30-active.md`。
- 同一天多次归档时，追加序号：`2026-04-30-active-2.md`。
- 归档范围：`active.md` 和 `status.md`（evidence/ 和 reviews/ 不归档，保留原位）。
...
| archived_reason | string | 归档原因：`user-restart`（用户选择重新开始） |
...
okr-planner ──读取 GM OKR + 角色树──写入──> active.md [## 层级 OKR, ## 交付幕计划]
    │                                  └──创建──> status.md
    ▼
okr-execution-plan ──读取 层级 OKR + 交付幕计划──更新──> active.md [## 交付幕计划]
    │
    ▼
...
okr-alignment-check ──读取 active.md──分析（只读）
    │
    ▼
okr-review-score ──读取 status.md + evidence/──写入──> reviews/
    │
    ▼
okr-next-cycle ──读取 reviews/ + status.md──建议（确认后更新 active.md）
```
...
| okr-execution-plan | `active.md` 的 `## 交付幕计划` 包含任务级明细 | 停止，报告执行计划生成失败 |
