...
> Agent Delivery OS for Claude Code and Codex: 把一句需求转成可执行、可验证、可复盘的 AI 团队交付闭环。

## 为什么是 DoWithOKR

DoWithOKR 不是 OKR 管理工具，也不是任务列表。它是一套 **Agent Delivery OS**：用 OKR 把 AI 编程从"执行动作"拉回"交付价值"。

普通 coding agent 很容易做出一串代码改动，却没人能稳定回答：这个改动服务哪个目标？验收标准是什么？证据在哪里？下次会不会更好？DoWithOKR 把这些问题协议化：

**用户是甲方，GM 是需求代理，AI 扮演一支完整的产品技术团队。** 需求经过 GM OKR、角色拆解、交付幕、证据收集、上级评分和 wisdom 复盘，最终收敛为可验证的交付结果。

### 它和普通 AI 项目管理工具的区别

| 常见工具 | DoWithOKR |
| --- | --- |
| 记录任务和状态 | 定义目标、角色、证据和评分 |
| AI 帮你写计划或总结 | AI 团队按 OKR 交付并留下证据 |
| 关注"做了什么" | 关注"交付了什么价值、达到什么标准" |
| 状态依赖平台数据库 | 状态保存在本地 `.okr/` 文件协议中 |
...
| Codex CLI | 创建项目本地 marketplace，symlink 技能、引用模板、脚本和 Web 资源 | `.agents/plugins/marketplace.json` + `.agents/plugins/plugins/dowithokr/` |
...
target-project/                     # 安装后的目标项目
  ...
  .agents/plugins/marketplace.json  # 项目本地 Codex marketplace
  .agents/plugins/plugins/dowithokr/
    .codex-plugin/plugin.json       # Codex 插件描述（复制）
