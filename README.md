...
1. 将技能文件复制到目标项目的 `.claude/commands/` 目录
2. 在目标项目的 `CLAUDE.md` 中追加技能路由规则

### Codex 安装

将 `DoWithOKR` 目录放到项目根目录下，Codex 会通过 `.codex-plugin/plugin.json` 自动发现并加载插件，无需额外配置。


 
### 手动安装

如果不使用安装脚本，可以手动将 `skills/*/SKILL.md` 复制为 `.claude/commands/okr-*.md`：

```bash
# 示例：手动复制技能文件
cp skills/okr-run/SKILL.md /path/to/your/project/.claude/commands/okr-run.md
cp skills/okr-boss/SKILL.md /path/to/your/project/.claude/commands/okr-boss.md
# ... 对每个技能重复
```
...
插件会在项目中创建 `.okr/` 目录，包含：
- `active.md` — OKR 主文件，记录当前 Boss OKR、角色树和各角色 OKR
- `status.md` — 状态看板，实时展示各 KR 的进展、证据和下一步

### 分步模式

除了全自动模式，你也可以逐步调用各技能精细控制流程：

```text
/okr-boss        → 将需求转成 Boss OKR
/okr-role-splitter → 拆解角色树
/okr-planner     → 生成层级 OKR 和交付幕计划
/okr-execution-plan → 输出执行计划
/okr-role-run    → 执行指定角色的 KR
```

### 状态文件

所有 OKR 状态保存在项目的 `.okr/` 目录中。建议将该目录添加到 `.gitignore`：

```bash
echo '.okr/' >> /path/to/your/project/.gitignore
```
